...
class Notifier:
    CMC = {
        ...
        'DOT':'polkadot','POL':'matic-network','ATOM':'cosmos',
    ...
    def entry(self, sym, side, price, sl, tp1, tp2, qty, stage, paper, passed=None, failed=None, score=0):
        ...
        if passed:
            body += f"GEC: {', '.join(passed[:5])}\n"
        body += f"{'='*24}"
        self._send(title, body, sym, priority='high', tags='chart_with_upwards_trend')
    ...
    def exit_notif(self, sym, reason, pnl, balance, entry_p, close_p, paper):
        ...
        title = f"{sonuc} -- {sym} {pnl:+.2f} USDT [{mode}]"
        body  = (
            f"{'='*24}\n"
            f"{sym} | {sonuc}\n"
            f"{'='*24}\n"
            f"Giris  : {entry_p:.4f}\n"
            f"Cikis  : {close_p:.4f}  ({chg:+.2f}%)\n"
            f"Sebep  : {reason}\n"
            f"{'='*24}\n"
            f"PNL    : {pnl:+.2f} USDT\n"
        ...
        self._send(title, body, sym, priority='high' if pnl > 0 else 'default', tags=tags)

    def partial_notif(self, sym, reasons, pnl):
        title = f"KISMI CIKIS -- {sym}"
        body  = (
            f"{'='*24}\n"
            f"{sym} | KISMI CIKIS\n"
            f"{'='*24}\n"
        ...
        self._send(title, body, sym, tags='warning')

    def stage2_notif(self, sym, side, price, qty, paper):
        mode  = 'PAPER' if paper else 'CANLI'
        title = f"2. KADEME -- {sym} [{mode}]"
        body  = (
            f"{'='*24}\n"
            f"{sym} | 2. KADEME GIRIS\n"
            f"{'='*24}\n"
            f"{side} @ {price:.4f}\n"
            f"Eklenen : {qty} USDT\n"
            f"Trend onaylandi\n"
    ...
    def daily_report(self, stats, paper, trades=None, start_bal=None):
        mode  = 'PAPER' if paper else 'CANLI'
        pnl   = stats['total_pnl']
        sonuc = 'YUKSELIS' if pnl >= 0 else 'DUSUS'
        title = f"GUNLUK RAPOR [{mode}] -- {sonuc}"
        body  = (
            f"{'='*24}\n"
            f"GUNLUK RAPOR [{mode}]\n"
